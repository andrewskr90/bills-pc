...
//     }
// }
const buildGetPortfolio = (userId, time, reqQuery) => {
    ...
    if (reqQuery && reqQuery.searchvalue) {
        additionalWhere += ` AND it.name LIKE '%${reqQuery.searchvalue}%'`
    } else if (reqQuery && reqQuery.itemId) {
        additionalWhere += ` AND it.id = '${reqQuery.itemId}'`
    }
    query += additionalWhere
    query += 'order by credit.time desc' // when order by becomes dynamic, tests need to include this
    const variables = []

    const pageInt = reqQuery ? parseInt(reqQuery.page) : undefined
...
const getPortfolio = async (userId, time, reqQuery) => {
    const getPortfolioQuery = buildGetPortfolio(userId, time, reqQuery)
...
module.exports = { buildGetPortfolio, getPortfolio }
