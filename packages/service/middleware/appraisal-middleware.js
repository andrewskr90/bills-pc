...
const createAppraisal = async (req, res, next) => {
    try {
        ...
        const collectedItem = await CollectedItem.getById(collectedItemId, req.claims.user_id)
        if (collectedItem.conditionId === conditionId) {
            return next({ status: 400, message: 'Appraisal condition must differ from current condition.' })
        }
        if (new Date(collectedItem.appraisalTime) > new Date(time)) {
        ...
        const postedAppraisalId = 'fakeId'
