import { getCardsBillsPc, getItemsBillsPc, getProductsBillsPc, getSetsBillsPc, loginBillsPc, postItemsBillsPc } from "./api/index.js";

const cookies = await loginBillsPc()
const sets = await getSetsBillsPc(cookies)

for (let i=0; i<sets.length; i++) {
    const curSet = sets[i]
    const existingItems = await getItemsBillsPc({ setId: curSet.set_v2_id }, cookies)
